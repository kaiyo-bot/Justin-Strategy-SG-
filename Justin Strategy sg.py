...
# TELEGRAM CONFIG
# ==================================================
TELEGRAM_BOT_TOKEN = "8837075159:AAFabwOY4DC2dtj-uHby-9AAQyc2usjrPWU"
TELEGRAM_CHAT_ID = "-1004326600232"
...
class GodTobiStrategyEngine:
    ...
    def get_prediction(self, logic_name, period):
        if logic_name == "TREND_FOLLOW":
            ...
            return "SMALL" if self.history[0]['type'] == "BIG" else "BIG" if len(self.history) > 0 else "BIG"
...
def fetch_history_list():
    """Fetch TRX history with browser-like headers and detailed 403 diagnostics."""
    try:
        ...
        if r.status_code == 403:
            body = r.text.replace("\n", " ").strip()
            logger.error("History API returned 403. Response body: %s", body[:1000])
            logger.error("Response headers: %s", dict(r.headers))
            return None
        ...
        logger.error("Unexpected History API response: %s", str(data)[:1000])
